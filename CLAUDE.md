...
- `.github/workflows/release.yml` — tag-driven cross-platform builds via `tauri-action` (macOS Apple Silicon + Intel `.dmg`, Linux `.AppImage` + `.deb`, Windows `.msi` + `.exe`). Fires on `v*` tags or manual `workflow_dispatch`. Maintainer recipe in [`docs/releases.md`](./docs/releases.md). Auto-update via `tauri-plugin-updater` (#10) is the natural follow-up — gated on a signing-key decision plus this pipeline producing the artefacts the updater plugin's manifest can point at.
