...
- **Logging has three sinks.** stderr fmt + daily-rolling file appender at `~/Library/Logs/io.github.khawkins98.hush/hush.log.<date>` (#624) + the in-app `DebugLogLayer` ring buffer (#532). `init_tracing` in `lib.rs` composes them. Disable the file sink with `HUSH_LOG_FILE=off`. The `ort` crate's own tracing is on (`features = ["tracing"]`) and routes through the same path. Settings → Debug surfaces the file path with reveal/copy controls (#627). Architecture detail: see `ARCHITECTURE.md` "Logging / observability".
