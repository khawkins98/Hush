...
- `src-tauri/src/hotkey/` — `tauri-plugin-global-shortcut` for the toggle hotkey + `rdev` for push-to-talk. PTT is **opt-in on macOS** via `HUSH_PTT_ENABLE=1`. The macOS-26 abort that originally drove the default-off behaviour (rdev calling `TISGetInputSourceProperty` from a non-main thread, hitting `dispatch_assert_queue_fail`) is fixed by pinning rdev to a git rev that includes [Narsil/rdev#147](https://github.com/Narsil/rdev/pull/147); the env gate stays so Input Monitoring is granted deliberately, not on first launch.
