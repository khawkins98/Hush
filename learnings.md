...
> ⚠ **Superseded by later finding.** A subsequent investigation ("2026-05-13 — CGEventTap auto-deny" below) found that on macOS 26, calling `CGEventTapCreate` while IM is `NotDetermined` auto-creates a TCC `Deny` entry — it does NOT fail gracefully. The current `lib.rs` gate skips PTT startup for both `NotDetermined` *and* `Denied`; PTT starts in-session only after the user explicitly grants IM via `request_input_monitoring_permission`. The "safe to attempt NotDetermined" conclusion in this entry is incorrect on macOS 26 and must not be replicated.
...
1. **Stale banner** — `permissions.svelte.ts` derives `anyPermsStale` from `get_permission_health` and computes `showStaleBanner` (true when any perm is stale, the banner hasn't been dismissed, and the user isn't already on the Permissions tab). `+page.svelte` renders `permissions.showStaleBanner`. An amber banner appears at the top of the main content area linking directly to Settings → Permissions.
